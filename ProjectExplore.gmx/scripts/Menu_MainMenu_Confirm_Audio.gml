...
draw_text(640,255*button_slide,string_split("Are you sure you would like to make these changes to your audio settings?",510));
...
case 0:{
obj_volume.xm = cache_x[0];
obj_volume.x1 = cache_x[1];
obj_volume.x2 = cache_x[2];
obj_volume.x3 = cache_x[3];
menu_state="Options";surface_update=true;button_slide=0;break;}
case 1:{
ini_open(working_directory + "audio.ini")
ini_write_real("Volume","xm",obj_volume.xm) //X MASTER start
ini_write_real("Volume","x1",obj_volume.x1) //X Music start
ini_write_real("Volume","x2",obj_volume.x2) //X Sound start
ini_write_real("Volume","x3",obj_volume.x3) //X Ambience start
ini_close();
